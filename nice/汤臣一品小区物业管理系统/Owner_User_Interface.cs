...
string nom = Form1.nom;//调用
   ...
   (
       ...
       int nHeightEllipse // width of ellipse
        ...
        {
            lab_nam.Text = info.num;
            //查询业主用户信息表
            string yzhuSql = string.Format("select * from Users where usersnum='{0}'", nom);
            DataRow dr = DBHepler.GetDataSet(yzhuSql).Tables[0].Rows[0];//创建新行
            //将表中的数据取到个人中心窗体的文本框中
            ...
            DataTable dt1 = DBHepler.GetDataTable(selectsql1);
            //遍历出所有的房间号
        ...
        {  //声明变量存储投诉时间
            ...
            else
            {
                ...
                {  //添加投诉信息SQL语句
                    ...
                    //判断数据是否添加成功
                    if ((DBHepler.ExecuteNonQuery(insertSql)) && (DBHepler.ExecuteNonQuery(intoSql)))
                    {
        ...
        {  //非空验证
            ...
            if (cbb_type.Text=="水费")
            {
                ...
                if (DBHepler.ExecuteNonQuery(sqlupdate1))
                {
            ...
            else if (cbb_type.Text=="电费")
            {
                ...
                if (DBHepler.ExecuteNonQuery(sqlupdate2))
                {
            ...
            else if (cbb_type.Text=="物业管理费")
            {
                ...
                if (DBHepler.ExecuteNonQuery(sqlupdate3))
                {
        ...
        private void About_Us_Click(object sender, EventArgs e)
        {
            ...
            //弹框提示关于程序
            MessageBox.Show("软件名称：汤臣一品小区物业管理系统\n工作室名称：小黑屋工作室\n工作室负责人：戴铖\n工作室成员：陈晨逸、李宇航、周婷立\n系统版本号：V 1.1.0\n发布日期：2019-06-14\n联系邮箱：Luacswork@163.com\n所有权：小黑屋工作室所有成员", "关于");
        }
        ...
        {
            //获取房间号存储在ID类中
            ID.id = info_dgv.SelectedRows[0].Cells["col_name"].Value.ToString();
            info.num = info_dgv.SelectedRows[0].Cells["col_id"].Value.ToString();
            if (ID.id == "")
            {
