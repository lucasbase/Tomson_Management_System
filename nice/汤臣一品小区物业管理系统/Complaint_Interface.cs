...
namespace 汤臣一品小区物业管理系统
{
    public partial class Complaint_Interface : Form
    {
        ...
        private void Complaint_Interface_Load(object sender, EventArgs e)
        {
            ...
            string selectsql = string.Format("select * from complain_register_table crt join complain_state_table cst on crt.complain_name=cst.complain_name where crt.complain_name='{0}'", ID.id);
            DataSet dt = DBHepler.GetDataSet(selectsql);
        ...
        private void btn_evaluate_Click(object sender, EventArgs e)
        {
            ...
            //更新SQL语句
            string updateSql = string.Format("update complain_state_table set evaluate='{0}' where complain_id='{1}'", evaluate_cbb.Text,info.num);
