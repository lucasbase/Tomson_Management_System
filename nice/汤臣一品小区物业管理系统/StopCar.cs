...
namespace 汤臣一品小区物业管理系统
{
    public partial class StopCar : Form
    {
            ...
            try
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (DBHepler.ExecuteNonQuery(sqlinto))
                        {
        ...
        private void btn_jz_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    //更新停车表中收费金额以及缴费状态
                    string updatesql = string.Format("update stopcar_table set state='已缴费',charge_amont='{0}' where stopcar_id='{1}'", tb_money.Text, id);
                    DBHepler.ExecuteNonQuery(updatesql);
                    //声明变量存储结算成功后的弹框提示信息
                    string MO = "";
        ...
        private void 离开ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (DBHepler.ExecuteNonQuery(updateSql))
                {
                    ...
                    DataSet ds = DBHepler.GetDataSet(selectSql);
        ...
        private void StopCar_Load(object sender, EventArgs e)
        {
            ...
            DataTable ds = DBHepler.GetDataTable(selectsql1);
            //遍历出所有的小区名
